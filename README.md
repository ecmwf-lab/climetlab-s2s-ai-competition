...
- **Training input** dataset. It consists in data from three different models : ECMWF (ecmf), ECCC (cwao), NCEP (eccc).
These data are hindcast data. This is used as the input for training the ML models.
The `training-input` dataset is available as grib, netcdf or zarr
...
- **Forecast input** dataset. It consists in data from three different models : ECMWF (ecmf), ECCC (cwao), NCEP (eccc).
...
Using data from earlier that 2020/01/01 is also allowed.
  - For all 3 models : 
  ...
  Question : cut the lead time to 32 days for all 3 models ?

- **Observations** dataset. This is a ground truth to compare with the ML model output and evaluate them. It consists in observation from instruments of temperature and total precipitation (TODO add more description). 
...
Zarr urls are :
  -  `training-input` https://storage.ecmwf.europeanweather.cloud/s2s-ai-competition/data/training-set/ecwmf/0.1.43/zarr/ TODO
  -  `forecast-input` https://storage.ecmwf.europeanweather.cloud/s2s-ai-competition/data/training-set/ecwmf/0.1.43/zarr/ TODO
  -  `observations` 
  -  `benchmark` 
